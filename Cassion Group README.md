...
- The delays from time.sleep() caused orders to finish out of order. Even though orders were dispatched sequentially, the randomized sleep durations (0.5–2.0 seconds) meant that a later order assigned to a fast worker could complete before an earlier one assigned to a slower worker. This is exactly what you'd see in a real system — network latency, database calls, or I/O would cause the same kind of non-deterministic completion order. The master handles this fine because it just waits for any result with MPI.ANY_SOURCE and stores whatever comes back.
...
- When running without a lock, multiple workers finishing at nearly the same time could both try to append to shared_orders at the same moment. This creates a race condition — the internal state of the list gets corrupted because one write can overwrite or interfere with another that hasn't completed yet. In testing, this sometimes caused duplicate entries, missing entries, or in some cases a crash. The results were inconsistent across runs, which makes the system unreliable.
